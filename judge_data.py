...
class ModelEvaluator:
    ...
    def load_test_set(self, path: str = "data/eval_conversations.json", limit: int = 1000) -> List[Dict[str, Any]]:
        ...
        for conv in test_conversations:
            ...
            if (len(context) >= 3 and # only those conversations with at least 3 turns.
        ...
        if valid_conversations:
            sample_conv = valid_conversations[0]
            print("Sample conversation context:")
            for turn in sample_conv.get('conversation_context', []):
                role = turn.get('role', '')
...
def main():
    ...
    model_parser.add_argument('--conversations', '-c', 
                             default='data/eval_conversations.json',
                             help='Path to conversations file (default: data/eval_conversations.json)')
